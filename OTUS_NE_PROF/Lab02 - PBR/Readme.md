...
Из Чокурдах у нас 2 линка в сторону ISP Триада. С целью проверки правильности выполнения поставленных задач далее будем пинговать и трассировать  лупбэк роутера R25 и R26, но и они должны  знать друг о друге, поэтому убедимся в этом: 
...
Создаем Access-List в который включим пока один хост, например VPC30
```
...
```
И вешаем Route-map на входящий интерфейс, в данном случае - сабинтерфейс e0/2.50
```
...
```
проверяем, как применилась политика:
```
...
```
Попробуем погасить интерфейс со стороны R26 и посмотрим на трассировку
```
R26>
R26>en
R26#conf t

...
# Топология
![](Topology.png)
![](MIckeyMouseTopology.png)

## Подготовка
Запускаем процесс ospf на всех устройствах, задаем RID по принципу X.X.X.X, где X - номер устройства, щапускаем ospf на интерфейсах, анонсируем сети и лупбэки:
...
```
И посмотрим, что он добрался до всех зон:
</code></pre>
</details>
<details>
...
Все выглядит рабочим, маршруты прилетели. Теперь перейдем к фильтрации :)

## Маршрутизаторы R14-R15 находятся в зоне 0 - backbone.

В принципе уже выполнено.
...
Обьявим зону 101 как Totally Stub Area, чем предотвратим отправку LSA 3 типа:
...
R19 получает только маршрут по умолчанию как внешний второго типа. Но со стороны R19 тоже надо обьявить зону как Totally stub
...
R19(config-router)#
```
и тогда таблица маршрутизации принимает вид:

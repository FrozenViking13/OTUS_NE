...
Как видим, число получаемых R22 маршрутов от R14 снизилось с 33 до 20.
...
router bgp 2042
 ...
 maximum-paths 2
```
Что примечательно, фактически в AS стыковычные сети начинаются с 10.10.10.40/30 и заканчиваются 10.10.10.64/30. В таблице присутствует 10.10.10.32/27, что как бы не оч правильно, хотя, конечно, кому они нужны? Поэтому я написал префикс лист так как написал, включив в него только реально задействованный диапазон адресов. Однако, после фильтрации:
<details>
...
... мы все равно видим 10.10.10.32/27... Ну, тем не менее, количество получаемых маршрутов снизилось с 12 до 10.
...
На R22 распространяем маршрут по умолчанию и вешаем пустой route-map на выход в сторону R14
```
R22#sh run | sec bgp
router bgp 101

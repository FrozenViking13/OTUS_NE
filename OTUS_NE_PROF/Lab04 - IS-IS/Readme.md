...
Было много теории и мало практики :)

## R23 и R25 находятся в зоне 2222.

Как я вижу - Поднимаем процесс ISIS, по умолчанию маршрутизаторы поднимаются в L1-L2 и вроде бы можно явно не указывать, но мы укажем. L1 у нас внутри зоны и L2 в другие зоны. 
...
Посмотрим  соседство и таблицу маршрутизации:
```
...
```
Проверим соседство и таблицу маршрутизации:
```
...
В таблице маршрутизации видим статику до 1.1.1.25, которую ставили при настройке PBR. Уберем и проверим.
...
```
Вот теперь все вроде в порядке :)
